...
test("the asset-cache hotfix is consistently versioned as 0.15.15", async () => {
  const manifest = JSON.parse(await readFile(manifestUrl, "utf8"));
  const readme = await readFile(readmeUrl, "utf8");
  const buildInfo = await readFile(buildInfoUrl, "utf8");

  assert.equal(manifest.version, "0.15.15");
  assert.match(readme, /version-0\.15\.15/);
  assert.match(readme, /Release 0\.15\.15/);
  assert.match(readme, /Novità 0\.15\.15/);
  assert.match(readme, /cache/iu);
  ...
  assert.match(buildInfo, /["']?integrationVersion["']?\s*:\s*["']0\.15\.15["']/);
  assert.match(buildInfo, /["']?dashboardVersion["']?\s*:\s*["']0\.15\.15["']/);
});
