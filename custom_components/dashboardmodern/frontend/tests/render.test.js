...
test("built-in registries are deterministic and compatibility renderer APIs report real types", () => {
  ...
  assert.deepEqual(cardRendererTypes(second), ["automation-control", "battery-status", "button-control", "camera-status", "climate-control", "compat-card", "cover-control", "energy-flows", "energy-overview", "fan-control", "grid-status", "home-summary", "input-boolean-control", "input-number-control", "input-select-control", "legacy-panel", "light-control", "media-player-control", "scene-control", "script-control", "sensor-status", "solar-production", "switch-control", "vacuum-control", "weather-current", "weather-forecast"]);
