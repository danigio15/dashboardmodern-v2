...
test("the persistence/mobile hotfix is consistently versioned as 0.15.14", async () => {
  const manifest = JSON.parse(await readFile(manifestUrl, "utf8"));
  const readme = await readFile(readmeUrl, "utf8");
  const buildInfo = await readFile(buildInfoUrl, "utf8");

  assert.equal(manifest.version, "0.15.14");
  assert.match(readme, /version-0\.15\.14/);
  assert.match(readme, /Release 0\.15\.14/);
  assert.match(readme, /Novità 0\.15\.14/);
  assert.match(readme, /Energia annuale/);
  assert.match(readme, /Brands/);
  ...
  assert.match(buildInfo, /["']?integrationVersion["']?\s*:\s*["']0\.15\.14["']/);
  assert.match(buildInfo, /["']?dashboardVersion["']?\s*:\s*["']0\.15\.14["']/);
});
...
test("local package brand assets remain valid while HACS catalog registration stays external", async () => {
  ...
  const readme = await readFile(readmeUrl, "utf8");
  assert.match(readme, /catalogo esterno Home Assistant Brands/);
});
