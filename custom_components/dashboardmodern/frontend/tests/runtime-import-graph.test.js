...
test("production graph is single-owner, acyclic and contains no facade pass-throughs", async () => {
  ...
  // regressions: explicit Casa period energy, multiple temperature associations
  // per room and canonical appliance artwork. It adds no polling or observer.
  // All facade/cycle/orphan/polling/global-observer checks stay active.
  assert.ok(relative.length <= 76, `production graph unexpectedly grew to ${relative.length} modules`);
  assertAcyclic(edges);
