  ...
  await page.evaluate(
    ({ solare, rete, batteria }) => {
      const scrivi = (id, valore) => {
        _RAW_STATES[id] = { entity_id: id, state: String(valore), attributes: { unit_of_measurement: "W" } };
...
test("rete → casa e rete → batteria, non «batteria che alimenta casa»", async ({ page }, testInfo) => {
  test.setTimeout(120_000);
  await page.route("https://**", (route) => route.fulfill({ status: 200, body: "" }));
  await bootNamespacedDashboard(page, "dashboard.html", testInfo, SEED);
  await page.locator("#setup-wizard").evaluateAll((nodes) => nodes.forEach((n) => n.remove()));
  await expect
    .poll(() => page.evaluate(() => window.__DASHBOARDMODERN_RUNTIME_ROOT__?.ready === true), { timeout: 60_000 })
    .toBe(true);
  ...
  await expect.poll(() => linee(page)).toMatchObject({
    "line-grid-home": true,
