...
export function createApplianceViewModel(device = {}, states = {}, rooms = [], locale = "it") {
  ...
  const unavailable = [powerEntity, controlEntity].filter(Boolean).some((id) => ["unknown", "unavailable"].includes(clean(states?.[id]?.state).toLowerCase()));
  ...
  const operational = ["playing", "heat", "cool", "open", "opening", "running", "active"].includes(controlState);
  const mode = unavailable && watts == null ? "unavailable" : operational || (watts != null && watts >= run) ? "running" : controlState === "on" || (watts != null && watts >= standby) ? "standby" : "off";
  ...
  return Object.freeze({
    ...
    watts, powerEntity, controlEntity, historyEntity,
