...
function polishLoadsEditor() {
  const body = doc?.getElementById("ed-body");
  if (!body || !["load", "loads"].includes(activeTab()) && body.dataset.renderer !== "loads") return false;
  ...
  const room = form.querySelector("#dm-load-room")?.closest(".ed-form-row") || form.querySelector("#dm-load-room")?.parentElement;
  ...
  form.querySelectorAll(":scope > label.ed-intro").forEach((label) => { label.dataset.loadGroup = "visibility"; });
...
function subscribeStore() {
  ...
  store.subscribe((change) => {
    if (["lights", "loads", "rooms", "entityOverrides", "snapshot"].includes(change?.section)) schedule();
  });
}
...
function installStyles() {
  installStyle("dm-editor-polish-style", `
    ...
    .dm-light-row{display:grid!important;grid-template-columns:70px minmax(230px,1fr) minmax(210px,320px) 48px 48px!important;grid-template-areas:"order main room edit delete"!important;align-items:center!important;gap:12px!important;min-width:0!important;min-height:82px!important;padding:12px!important;background:var(--secondary-background-color,#f6f8fb)!important;border:1px solid var(--divider-color,#dbe4ee)!important;border-radius:15px!important;overflow:visible!important}
