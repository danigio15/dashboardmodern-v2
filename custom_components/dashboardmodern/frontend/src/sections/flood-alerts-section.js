 ...
 */
export function floodEntities(extras = {}, removed = {}, states = {}) {
  ...
  return { entities: uscita, primoAvvio: !Array.isArray(dichiarate) };
}
...
export function syncFloodGroup() {
  ...
  const { entities, primoAvvio } = floodEntities(extras, removed, allStates());
  /* Il primo avvio scrive quello che ha trovato: da li' in poi la lista e'
   * dell'utente, e chi la svuota la ritrova vuota invece di vedersela
   * ripopolare a ogni ricaricamento. */
  if (primoAvvio && entities.length) {
