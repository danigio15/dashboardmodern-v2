import { elementWidth, measuredGap, navigationEntriesForSections, normalizeNavigationConfig, partitionItemsByMeasuredWidth, reducedMotion, shouldHideOnScroll, revealOnScroll } from "./navigation.js";
...
export class BottomNavigationController {
  constructor(container, store, { ResizeObserverClass = globalThis.ResizeObserver, setTimer = setTimeout, clearTimer = clearTimeout, raf = globalThis.requestAnimationFrame } = {}) { this.container = container; this.store = store; this.ResizeObserverClass = ResizeObserverClass; this.setTimer = setTimer; this.clearTimer = clearTimer; this.raf = raf || ((fn) => fn()); this.lastY = 0; this.hideTimer = null; this.bound = []; this.resizeObserver = null; this.lastActiveSectionId = null; this.currentScroller = null; this.scrollerHandler = null; this.edgeFrame = false; }
  ...
  destroy() { for (const [target, type, fn, options] of this.bound) target?.removeEventListener?.(type, fn, options); this.bound = []; this.disconnectScroller(); this.resizeObserver?.disconnect?.(); this.cancelHide(); }
  ...
  refresh() { this.connectScroller(); this.recalculate(); const nav = this.nav(); if (!nav) return; if (nav.className.includes("is-auto-hide")) this.scheduleHide(); else { this.cancelHide(); nav.dataset.hidden = "false"; } }
  disconnectScroller() { if (this.currentScroller && this.scrollerHandler) this.currentScroller.removeEventListener?.("scroll", this.scrollerHandler); this.currentScroller = null; this.scrollerHandler = null; }
  connectScroller() { const scroller = this.scrollArea(); if (scroller === this.currentScroller) return; this.disconnectScroller(); this.currentScroller = scroller; this.scrollerHandler = () => { if (this.edgeFrame) return; this.edgeFrame = true; this.raf(() => { this.edgeFrame = false; this.updateEdges(); }); }; scroller?.addEventListener?.("scroll", this.scrollerHandler, { passive: true }); }
  recalculate() { const nav = this.nav(); const scroller = this.scrollArea(); if (!nav || !scroller) return; const more = nav.querySelector?.("[data-nav-more]"); const items = [...scroller.querySelectorAll?.('[data-nav-menu-item="false"]') || []]; const gap = measuredGap(scroller, 8); const total = items.reduce((sum, item, index) => sum + elementWidth(item) + (index > 0 ? gap : 0), 0); const available = elementWidth(scroller) || scroller.clientWidth || nav.clientWidth || 0; const fits = !available || total <= available + 1; nav.dataset.overflowing = String(!fits); scroller.style.overflowX = fits ? "hidden" : "auto"; if (nav.className.includes("overflow-more-menu") && !fits) { more.hidden = false; const moreWidth = elementWidth(more.querySelector?.("summary")) || elementWidth(more) || 64; const moreGap = measuredGap(nav, gap); const parts = partitionItemsByMeasuredWidth(items, available, moreWidth, gap, moreGap); const visibleIds = new Set(parts.visible.map((item) => item.dataset.sectionNavId)); const overflowIds = new Set(parts.overflow.map((item) => item.dataset.sectionNavId)); for (const item of items) item.hidden = !visibleIds.has(item.dataset.sectionNavId); for (const item of more.querySelectorAll?.("[data-section-nav-id]") || []) item.hidden = !overflowIds.has(item.dataset.sectionNavId); const summary = more.querySelector?.("[data-nav-more-summary]"); if (summary) summary.textContent = parts.overflow.some((item) => item.dataset.sectionNavId === this.store?.state?.activeSectionId) ? "More •" : "More"; } else { more.hidden = true; for (const item of items) item.hidden = false; }
  ...
  syncActive() { const current = this.store?.state?.activeSectionId; if (current !== this.lastActiveSectionId) { this.lastActiveSectionId = current; this.reveal(); } this.refresh(); }
  ...
  handlePointer(event) { const rect = this.container?.getBoundingClientRect?.(); const bottom = Number.isFinite(rect?.bottom) ? rect.bottom : (this.container?.clientHeight || globalThis.innerHeight || 0); if (bottom && event?.clientY >= bottom - 80) this.reveal(); }
  handleClick(event) { const button = event.target?.closest?.("[data-section-nav-id]"); if (!button) return; this.cancelHide(); const sectionId = button.dataset.sectionNavId; this.store?.setActiveSection?.(sectionId); this.findSectionNode(sectionId)?.scrollIntoView?.({ block: "start", inline: "nearest", behavior: reducedMotion() ? "auto" : "smooth" }); }
  findSectionNode(sectionId) { const walk = (node) => { if (!node) return null; if (node.dataset?.sectionId === sectionId) return node; for (const child of node.children || []) { const found = walk(child); if (found) return found; } return null; }; return walk(this.container); }
