...
export function partitionItemsByMeasuredWidth(items, availableWidth, moreWidth) { let used = Math.max(0, moreWidth); let count = 0; for (const item of items) { const width = elementWidth(item); if (used + width > availableWidth) break; used += width; count += 1; } return { visible: items.slice(0, count), overflow: items.slice(count) }; }
