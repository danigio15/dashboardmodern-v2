...
function editorRenderAppliances(){
  ...
  const rows=list.map((a,i)=>{
    ...
    return '<div class="ed-row"><div class="ed-row-main"><div class="ed-row-new"><span style="display:inline-flex;vertical-align:middle;color:#0ea5e9;">'+cdApplianceIcon(a.icon,18)+'</span> '+cdEscHtml(a&&a.name?a.name:cdApplianceName(a.icon))+' <span style="color:var(--text-dim,#94a3b8);font-size:11px;font-weight:700;">· '+ents.length+' entità</span></div><div class="ed-row-old mono">'+cdEscHtml(ents.join(', '))+'</div></div><div class="ed-del" onclick="edApplEdit('+i+')" style="color:#0ea5e9;" title="Modifica">✏️</div><div class="ed-del" onclick="edApplDel('+i+')" title="Elimina">🗑️</div></div>';
...
function editorRenderAvvisi() {
    ...
    if (customList.length) {
        const cinner = customList.map((a, i) => {
            const ents = a.entities || (a.entity ? [a.entity] : []);
            return `<div class="ed-row"><div class="ed-row-main"><div class="ed-row-new">${cdEscHtml(a.icon||'⚠️')} ${cdEscHtml(a.name||ents[0]||'')} <span style="color:var(--text-dim,#94a3b8); font-size:11px; font-weight:700;">· ${cdAvvisoCondLabel(a)}${ents.length>1?' · '+ents.length+' entità':''}</span></div><div class="ed-row-old mono">${cdEscHtml(ents.join(', '))}</div></div><div class="ed-del" onclick="edEditAvvisoCustom(${i})" style="color:#0ea5e9;" title="Modifica">✏️</div><div class="ed-del" onclick="edDelAvvisoCustom(${i})" title="Elimina">🗑️</div></div>`;
        }).join('');
    ...
    return `
      ...
      <details class="ed-acc"><summary class="ed-acc-head">💡 Luci <span class="ed-acc-n">${Object.keys((function(){ try { return JSON.parse(localStorage.getItem('cd_luci')) || {}; } catch(e) { return {}; } })()).length}</span></summary>
        <div class="ed-acc-body">
          ...
          ${(function(){ let L = {}; try { L = JSON.parse(localStorage.getItem('cd_luci')) || {}; } catch(e) {} return Object.entries(L).map(([id, nm]) => `<div class="ed-row"><div style="font-size:15px;">💡</div><div class="ed-row-main"><div class="ed-row-new">${cdEscHtml(nm)}</div><div class="ed-row-old mono">${id}</div></div><div class="ed-del" onclick="edDelLuce('${id}')">🗑️</div></div>`).join('') || '<div class="ed-empty">Nessuna luce configurata</div>'; })()}
          <div style="display:flex; gap:8px; margin:10px 0 8px;">
